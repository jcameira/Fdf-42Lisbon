...
/*   Updated: 2024/02/15 16:32:23 by jcameira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	invert_color(t_point *point)
{
	...
	updated_color[T] = -((point->color >> 24 & 0xFF) - 255);
	updated_color[R] = -((point->color >> 16 & 0xFF) - 255);
	updated_color[G] = -((point->color >> 8 & 0xFF) - 255);
	updated_color[B] = -((point->color & 0xFF) - 255);
	i = -1;
	while (++i < 4)
		if (updated_color[i] == 0)
			updated_color[i] = 1;
	point->color = (updated_color[T] << 24) + (updated_color[R] << 16) + (updated_color[G] << 8) + updated_color[B];
}
...
void	choose_color_scheme(t_map *map)
{
	...
	{
		map->color_scheme.bottom_color = ALMOST_BLACK;
		map->color_scheme.bottom_transition_color = DEEP_OCEAN;
		map->color_scheme.mid_color = GREYISH_BLUE;
		map->color_scheme.top_transition_color = GREENISH_GREY;
		map->color_scheme.top_color = ALMOST_WHITE;
	}
	if (map->scheme == SOFTER_TONES)
	{
		map->color_scheme.bottom_color = DARK_GREY;
		map->color_scheme.bottom_transition_color = GREYISH_GREEN;
		map->color_scheme.mid_color = BLUEISH_GREY;
		map->color_scheme.top_transition_color = SOFT_PINK;
		map->color_scheme.top_color = EGGSHELL;
	}
	if (map->scheme == BRIGHT_TONES)
	{
		map->color_scheme.bottom_color = REDISH_GREY;
		map->color_scheme.bottom_transition_color = GREY_BLUE;
		map->color_scheme.mid_color = GREY;
		map->color_scheme.top_transition_color = ORANGE;
		map->color_scheme.top_color = WHITE;
	}
	if (map->scheme == HIGH_CONTRAST)
	{
		map->color_scheme.bottom_color = VERY_DARK_BLUE;
		map->color_scheme.bottom_transition_color = BRIGHT_AQUA;
		map->color_scheme.mid_color = BRIGHT_RED;
		map->color_scheme.top_transition_color = BRIGHT_ORANGE;
		map->color_scheme.top_color = WHITE;
	}
	if (map->scheme == ANOTHER_EARTH)
	{
		map->color_scheme.bottom_color = MORE_DARK_BLUE;
		map->color_scheme.bottom_transition_color = GREENISH_BLUE;
		map->color_scheme.mid_color = BROWN;
		map->color_scheme.top_transition_color = BRIGHTER_ORANGE;
		map->color_scheme.top_color = ANOTHER_EGGSHELL;
	}
	if (map->scheme == BRIGHT_PASTEL)
	{
		map->color_scheme.bottom_color = PASTEL_OCEAN;
		map->color_scheme.bottom_transition_color = PASTEL_AQUA;
		map->color_scheme.mid_color = PASTEL_SALMON;
		map->color_scheme.top_transition_color = PASTEL_YELLOW;
		map->color_scheme.top_color = PASTEL_ORANGE;
	}
	if (map->scheme == PINKISH_PASTEL)
	{
		map->color_scheme.bottom_color = MORE_GREYISH_BLUE;
		map->color_scheme.bottom_transition_color = DARK_PURPLE;
		map->color_scheme.mid_color = PINKISH_WINE;
		map->color_scheme.top_transition_color = PINKISH;
		map->color_scheme.top_color = SOFT_SALMON;
	}
	set_point_color(map);
}
