...
/*   Updated: 2024/02/14 00:14:54 by jcameira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++y < map->limits[Y])
	{
		...
		while (++x < map->limits[X])
		{
			...
			if (round(map->points[y][x].coord[Z]) == map->z_max)
				...
				map->points[y][x].color = update_color_gradient(WHITE, FULL_GREEN, map->z_max, map->z_max - map->points[y][x].coord[Z]);
			else if (round(map->points[y][x].coord[Z]) < 0)
				map->points[y][x].color = update_color_gradient(FULL_BLUE, FULL_GREEN, - map->z_min, - map->z_min + map->points[y][x].coord[Z]);
		}
	}
}
