...
/*   Updated: 2024/02/12 20:51:49 by jcameira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_point_color(t_map *map)
{
	...
	while (++y < map->limits[Y])
	{
		...
		while (++x < map->limits[X])
		{
			...
			//printf("Z_value: %f\n", map->points[y][x].coordinates[Z]);
			if (round(map->points[y][x].coordinates[Z]) == map->z_max)
				map->points[y][x].color = WHITE;
			else if (round(map->points[y][x].coordinates[Z]) == map->z_min && round(map->points[y][x].coordinates[Z]) != 0)
				map->points[y][x].color = FULL_BLUE;
			else if (round(map->points[y][x].coordinates[Z]) == 0)
